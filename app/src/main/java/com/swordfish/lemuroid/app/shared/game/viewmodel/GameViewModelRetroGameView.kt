...
import com.swordfish.lemuroid.app.shared.game.view.RetroView
...
class GameViewModelRetroGameView(
    ...
    fun createEmptyRetroView(context: Context, lifecycle: LifecycleOwner) : GLRetroView{
        val result =
            RetroView(context, null)
                .apply {
                    isFocusable = false
    ...
    fun setupRetroView(retroView: GLRetroView) {
        val currentState = gameState.value
        if (currentState !is GameState.Loaded) throw IllegalStateException("Game is not loaded.")
        currentState.retroViewData

        (retroView as RetroView).setup(currentState.retroViewData)

//        lifecycle.lifecycle.addObserver(result)

        if (BuildConfig.DEBUG) {
            runCatching {
                printRetroVariables(retroView)
            }
        }
    }
    ...
    fun createRetroView(
        ...
        val result =
            RetroView(context, currentState.retroViewData)
                .apply {
                    isFocusable = false
                    isFocusableInTouchMode = false
                }
