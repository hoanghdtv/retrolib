...
open class GLRetroView(
    context: Context,
    protected var data: GLRetroViewData?
...
//    }
    ...
    protected var openGLESVersion: Int = 2

    protected var isGameLoaded = false
    protected var isEmulationReady = false
    protected var isAborted = false

    protected val retroGLEventsSubject = MutableSharedFlow<GLRetroEvents>(1)
    protected val retroGLIssuesErrors = MutableSharedFlow<Int>(1)

    protected val rumbleEventsSubject = MutableSharedFlow<RumbleEvent>()
    ...
    open fun onCreate(lifecycleOwner: LifecycleOwner) = catchExceptions {
    ...
    protected fun getDeviceLanguage() = Locale.getDefault().language

    protected fun getDefaultRefreshRate(): Float {
    ...
    protected fun clamp(x: Float, min: Float, max: Float) = minOf(maxOf(x, min), max)

    protected fun normalizeTouchCoordinates(x: Float, y: Float): PointF {
    ...
    protected fun getGLESVersion(context: Context): Int {
    ...
    private inner class RenderLifecycleObserver : LifecycleObserver {
        ...
        protected fun resume() = catchExceptions {
        ...
        protected fun pause() = catchExceptions {
    ...
    protected fun initializeCore() = catchExceptions {
    ...
    protected fun loadGameFromVirtualFiles(virtualFiles: List<VirtualFile>) {
    ...
    protected fun loadGameFromBytes(gameFileBytes: ByteArray) {
        LibretroDroid.loadGameFromBytes(gameFileBytes)
    }

    protected fun loadGameFromPath(gameFilePath: String) {
        LibretroDroid.loadGameFromPath(gameFilePath)
    }

    fun catchExceptions(block: () -> Unit) {
        try {
            if (isAborted) return
            block()
    ...
    protected fun <T> runOnGLThread(block: () -> T): T {
    ...
    protected fun buildShader(config: ShaderConfig): GLRetroShader {
        return when (config) {
    ...
    protected fun toParam(param: Float): String {
        return param.toString()
    }

    protected fun toParam(param: Boolean): String {
        return if (param) {
            "1"
        } else {
            "0"
    ...
    protected fun toParam(param: Int): String {
        return param.toString()
    }

    protected fun buildParams(vararg pairs: Pair<String, String?>): Map<String, String> {
        return pairs
    ...
    protected fun sendRumbleEvent(port: Int, strengthWeak: Float, strengthStrong: Float) {
        lifecycle?.coroutineScope?.launch {
            rumbleEventsSubject.emit(RumbleEvent(port, strengthWeak, strengthStrong))
        }
    }

    protected fun refreshAspectRatio() {
        runOnGLThread {
